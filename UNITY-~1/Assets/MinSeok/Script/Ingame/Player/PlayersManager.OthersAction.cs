...
public partial class PlayersManager : MonoBehaviour
{
    ...
    public void Action_Idle(int _index, Vector3 _pos)  //서있을때
    {
        ...
        // 포지션 동기화
        if (tf_players[myIndex].localPosition.x != _pos.x ||
           tf_players[myIndex].localPosition.z != _pos.z)
        {
            tf_players[myIndex].localPosition = Vector3.MoveTowards(tf_players[myIndex].localPosition, _pos,
            Time.smoothDeltaTime * speed[myIndex] * C_Global.interpolation_Pos);
        }

        // 로테이션 동기화(0.5도이내면 수행, 오일러 앵글로 검사)
        if (Mathf.Abs(tf_players[myIndex].localEulerAngles.y - networkManager.GetRotY(_index)) > 0.5f)
        {
        ...
        else
        {
            ...
            if (Mathf.Abs(tf_players[myIndex].localEulerAngles.y - networkManager.GetRotY(_index)) > 0.1f)
            {
                tmpRot.y = networkManager.GetRotY(_index);
                tf_players[myIndex].localEulerAngles = tmpRot;    // localEulerAngles값을 변경하면 localRotation값도 자동으로 변경된다.
            }
