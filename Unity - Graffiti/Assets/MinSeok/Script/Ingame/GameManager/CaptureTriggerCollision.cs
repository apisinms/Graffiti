...
public class CaptureTriggerCollision : MonoBehaviour
{
	...
	private int curCapturingPlayerIdx; //현재 점령중인 인덱스 저장
	private int myIndex;
	...
	private void OnTriggerStay(Collider other) //스테이지만 아래 예외에맞게 한번만 실행, 한번밟으면 다른애는 못들어옴
	{
		int idx = 0;
		switch (other.tag)
		{
		...
		curCapturingPlayerIdx = idx;
		//들어온놈이 가만히 서있을때(아이들)를 검사할것임. 서있으면 점령시작 아니면 모든 점령 코루틴을 끔.
		curCheckIdleCor = StartCoroutine(Cor_CheckPlayerOnlyIdle(curCapturingPlayerIdx, other.tag));
	}
	...
	private void OnTriggerExit(Collider other)
	{
		...
		if (curCapturingPlayerIdx != idx) //들어왔던애와 같은애가 나갔을때만 점령을 멈춤.
			return;
