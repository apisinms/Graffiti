...
{
#ifdef DEBUG
	static const int WEAPON_SELTIME = 4;   // 무기 선택 시간(초 단위)
#else
...
private:
	...
	void PackPacket(char* _setptr, int _num1, double _time, int& _size);
...
public:
	...
	void LoadingTimeChecker(RoomInfo* _room, double _loadingTimeElapsed);
	void RespawnChecker(RoomInfo* _room);
	void CarSpawnChecker(RoomInfo* _room, double _IngameTimeElapsed);
