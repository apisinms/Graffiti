...
public class InputInfoManager : MonoBehaviour
{
	...
	public InputField inputField_login_id, inputField_login_pw, 
		inputField_join_id, inputField_join_pw, inputField_join_nickname;
	...
	void Update()
	{
		...
		if (inputField_join_pw.isFocused == true)
		{
			if (Input.GetKeyDown(KeyCode.Tab))
			{
				inputField_join_nickname.Select();
			}
		}
        if (inputField_join_nickname.isFocused == true)
        {
            if (Input.GetKeyDown(KeyCode.Tab))
            {
	...
	public void BtnEnterLogin() //로그인 정보 입력후 확인버튼
	{
        ...
        //로그인정보가 틀렸다면 아래작성
        if (NetworkManager.GetInstance.CheckIDError() == true)
        {
            inputField_login_id.text = null;
            inputField_login_pw.text = null;
        ...
        if (NetworkManager.GetInstance.CheckPWError() == true)
        {
            inputField_login_id.text = null;
            inputField_login_pw.text = null;
            txt_result.text = retMsg;
        ...
        //로그인이 성공하면 아래작성
        if (NetworkManager.GetInstance.CheckLoginSuccess() == true)
        {
	...
	public void BtnEnterJoin() //회원가입 정보 입력후 확인버튼
	{
        ...
        Debug.Log("입력한 회원가입 NICKNAME: " + inputField_join_nickname.text);

        string retMsg = NetworkManager.GetInstance.MayIJoin(inputField_join_id.text, inputField_join_pw.text, inputField_join_nickname.text);

        Debug.Log("회원가입 결과 : " + retMsg);

        // 회원가입 정보가 틀렸다면 아래작성
        if (NetworkManager.GetInstance.CheckIDExit() == true)
        {
            inputField_join_id.text = null;
            inputField_join_pw.text = null;
            txt_result.text = retMsg;
